...
int main(int argc, char **argv) {
    ...
    uint64_t memory[2] = {UINT64_C(0xaaaaaaaaaaaaaaaa), UINT64_C(0xbbbbbbbbbbbbbbbb)};
    ...
    printf("AAPCS64 return=%016" PRIx64 " memory=%016" PRIx64 "%016" PRIx64,
           observation.result, memory[0], memory[1]);
    ...
    int status = result == expected_result && observation.result == expected_result &&
                 memory[0] == expected0 && memory[1] == expected1 && observation.sp != 0;
    for (size_t index = 0; index < 11; ++index) {
