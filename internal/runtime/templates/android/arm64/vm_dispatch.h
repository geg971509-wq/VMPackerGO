...
__attribute__((noinline)) VM_SECTION_SYSTEM static u32 hw_pauth(vm_ctx_t *vm) {
  return h_pauth(vm);
}
__attribute__((noinline)) VM_SECTION_SYSTEM static u32 hw_barrier(vm_ctx_t *vm) {
  return h_barrier(vm);
}
__attribute__((noinline)) VM_SECTION_SYSTEM static u32 hw_atomic(vm_ctx_t *vm) {
  return h_atomic(vm);
}
__attribute__((noinline)) VM_SECTION_SYSTEM static u32 hw_exclusive(vm_ctx_t *vm) {
  return h_exclusive(vm);
}
__attribute__((noinline)) VM_SECTION_SYSTEM static u32 hw_fpsimd(vm_ctx_t *vm) {
  return h_fpsimd(vm);
}
...
__attribute__((noinline)) VM_SECTION_BRANCH static u32 hw_jcond(vm_ctx_t *vm) {
  return h_jcond(vm);
}
__attribute__((noinline)) VM_SECTION_BRANCH static u32 hw_cbz(vm_ctx_t *vm) {
  return h_cbz(vm);
}
__attribute__((noinline)) VM_SECTION_BRANCH static u32 hw_cbnz(vm_ctx_t *vm) {
  return h_cbnz(vm);
}
...
__attribute__((noinline)) VM_SECTION_ALU static u32
hw_s_add_flags(vm_ctx_t *vm) {
  return h_s_add_flags(vm);
}
__attribute__((noinline)) VM_SECTION_ALU static u32
hw_s_sub_flags(vm_ctx_t *vm) {
  return h_s_sub_flags(vm);
}
__attribute__((noinline)) VM_SECTION_ALU static u32
hw_s_and_flags(vm_ctx_t *vm) {
  return h_s_and_flags(vm);
}
__attribute__((noinline)) VM_SECTION_ALU static u32
hw_s_adc_flags(vm_ctx_t *vm) {
  return h_s_adc_flags(vm);
}
__attribute__((noinline)) VM_SECTION_ALU static u32
hw_s_sbc_flags(vm_ctx_t *vm) {
  return h_s_sbc_flags(vm);
}
...
__attribute__((noinline)) static void vm_init_jump_table(vm_handler_fn *tbl) {
  ...
  /* 系统 */
  tbl[OP_NOP] = hw_nop;
  tbl[OP_PAUTH] = hw_pauth;
  tbl[OP_BARRIER] = hw_barrier;
  tbl[OP_ATOMIC] = hw_atomic;
  tbl[OP_EXCLUSIVE] = hw_exclusive;
  tbl[OP_FPSIMD] = hw_fpsimd;
  tbl[OP_HALT] = hw_halt;
  tbl[OP_RET] = hw_ret;
  ...
  tbl[OP_S_ADD_FLAGS] = hw_s_add_flags;
  tbl[OP_S_SUB_FLAGS] = hw_s_sub_flags;
  tbl[OP_S_AND_FLAGS] = hw_s_and_flags;
  tbl[OP_S_ADC_FLAGS] = hw_s_adc_flags;
  tbl[OP_S_SBC_FLAGS] = hw_s_sbc_flags;
