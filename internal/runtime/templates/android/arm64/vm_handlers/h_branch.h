...
/* Forward targets identify an opcode byte in [0, bc_len). Reverse targets
 * identify the byte immediately after an instruction and therefore live in
 * (0, bc_len]. */
static inline int vm_branch_target_valid(vm_ctx_t *vm, u32 target) {
  int valid = vm->reverse ? (target != 0 && target <= vm->bc_len)
                          : (target < vm->bc_len);
  if (valid)
    return 1;
...
static inline u32 h_je(vm_ctx_t *vm) {
  u32 target = rd32(&vm->bc[vm->pc + 1]);
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (vm->FL & FL_Z) vm->pc = target; else BRANCH_FALLTHROUGH(vm, 5);
  return 0;
}
...
static inline u32 h_jl(vm_ctx_t *vm) {
  u32 target = rd32(&vm->bc[vm->pc + 1]);
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (!!(vm->FL & FL_N) != !!(vm->FL & FL_V)) vm->pc = target;
  else BRANCH_FALLTHROUGH(vm, 5);
  return 0;
}
...
static inline u32 h_jb(vm_ctx_t *vm) {
  u32 target = rd32(&vm->bc[vm->pc + 1]);
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (!(vm->FL & FL_C)) vm->pc = target; else BRANCH_FALLTHROUGH(vm, 5);
  return 0;
}
...
static inline u32 h_jbe(vm_ctx_t *vm) {
  u32 target = rd32(&vm->bc[vm->pc + 1]);
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (!(vm->FL & FL_C) || (vm->FL & FL_Z)) vm->pc = target;
  else BRANCH_FALLTHROUGH(vm, 5);
  return 0;
}
...
static inline u32 h_tbz(vm_ctx_t *vm) {
  ...
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (!(vm->R[reg & 31] & ((u64)1 << (bit & 63)))) vm->pc = target;
  else BRANCH_FALLTHROUGH(vm, 7);
  return 0;
}
...
static inline u32 h_jcond(vm_ctx_t *vm) {
  ...
  if (!vm_branch_target_valid(vm, target)) return 0;
  if (vm_cond_holds(vm, cond)) vm->pc = target;
  else BRANCH_FALLTHROUGH(vm, 6);
  return 0;
}
...
static inline u32 h_cbz(vm_ctx_t *vm) {
  ...
  if (value == 0) vm->pc = target; else BRANCH_FALLTHROUGH(vm, 6);
  return 0;
}
