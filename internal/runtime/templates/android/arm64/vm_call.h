...
static inline u64 vm_file_bias(vm_ctx_t *vm) {
  u64 file_va = *(volatile u64 *)&_image_file_va;
  if (file_va == 0)
    return 0;
  return vm->image_anchor - file_va;
}
...
static inline void vm_apply_trailer(vm_ctx_t *vm, u8 *bc_buf, u32 bc_len) {
    ...
    return;
  {
    ...
    if (trail_func_addr != 0 && trail_map_count > 0 &&
      ...
      u32 j;
      vm->func_addr = trail_func_addr + vm_file_bias(vm);
...
static inline int vm_try_packed_tail(vm_ctx_t *vm, u64 addr) {
  u32 func_id;
  u8 *old_bc_buf;
  u32 old_bc_alloc;
  if (!vm_lookup_packed(vm, addr, &func_id))
    return 0;
  old_bc_buf = vm->bc_buf;
  old_bc_alloc = vm->bc_alloc;
  if (!vm_load_func(vm, func_id)) {
