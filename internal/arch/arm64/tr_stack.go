...
// sDup duplicate TOS
func (t *Translator) sDup() { t.emitOp(vm.OpSDup) }

// sSwap swap TOS and TOS-1
func (t *Translator) sSwap() { t.emitOp(vm.OpSSwap) }

// sDrop pop and discard TOS
func (t *Translator) sDrop() { t.emitOp(vm.OpSDrop) }

// ---- 栈模式翻译函数 ----

// trStackAluReg 翻译三寄存器 ALU (栈模式)
// ARM64: op Xd, Xn, Xm  →  VLOAD(rn) VLOAD(rm) S_OP VSTORE(rd)
func (t *Translator) trStackAluReg(inst vm.Instruction, sOp vm.Opcode) error {
	...
	if !inst.SF {
		t.emitOp(vm.OpSTrunc32) // W 寄存器模式截断
	}
...
func (t *Translator) trStackAluRegFlags(inst vm.Instruction, sOp vm.Opcode, setFlags bool) error {
	rd, rn, rm, err := t.mapReg3(inst)
	if err != nil {
		return err
	}
...
func (t *Translator) trStackAluImm(inst vm.Instruction, sOp vm.Opcode) error {
	return t.trStackAluImmFlags(inst, sOp, false)
}

// trStackAluImmFlags 翻译寄存器+立即数 ALU + 标志位 (栈模式)
func (t *Translator) trStackAluImmFlags(inst vm.Instruction, sOp vm.Opcode, setFlags bool) error {
...
func (t *Translator) trStackUnary(inst vm.Instruction, sOp vm.Opcode) error {
	rd, err := t.mapReg(inst.Rd)
	if err != nil {
		return err
	}
...
func (t *Translator) trStackMADD(inst vm.Instruction, isSub bool) error {
	...
	if isSub {
		t.emitOp(vm.OpSSub) // Ra - (Rn*Rm)
	} else {
		t.emitOp(vm.OpSAdd) // Ra + (Rn*Rm)
	}
	...
	case 3: // ROR
		if !sf {
			...
			if shift != 0 {
				t.sDup() // dup value
				t.sPushImm32(shift)
				t.emitOp(vm.OpSShr) // value >> shift
				t.sSwap()           // bring original value up
				t.sPushImm32(32 - shift)
				t.emitOp(vm.OpSShl) // value << (32-shift)
				t.emitOp(vm.OpSOr)  // combine
			}
		} else {
			t.sPushImm32(amount)
			t.sPushImm32(64)
...
func (t *Translator) trStackBitLogicalNot(inst vm.Instruction, sOp vm.Opcode, setFlags bool) error {
	rd, rn, rm, err := t.mapReg3(inst)
	if err != nil {
		return err
	}
...
func (t *Translator) trStackAddSubExt(inst vm.Instruction, sOp vm.Opcode, setFlags bool) error {
	rd, err := t.mapReg(inst.Rd)
	if err != nil {
		return err
	}
