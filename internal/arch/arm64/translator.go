...
// emitOp 将语义 opcode 映射为 wire opcode 后追加操作数。
func (t *Translator) emitOp(op vm.Opcode, operands ...byte) {
	wire, err := t.opcodes.Wire(op)
	if err != nil {
		panic(fmt.Sprintf("arm64 translator invariant: invalid semantic opcode %d: %v", op, err))
	}
	t.emit(wire)
	t.emit(operands...)
}
...
func (t *Translator) mapReg(arm64Reg int) (byte, error) {
	...
	if arm64Reg < 0 || arm64Reg > 31 {
		return 0, fmt.Errorf("register X%d is outside the VM range", arm64Reg)
	}
...
func (t *Translator) Translate(instructions []vm.Instruction) (*TranslateResult, error) {
	...
	for _, fix := range t.fixups {
		...
		if !ok {
			return nil, fmt.Errorf("ARM64 branch target offset 0x%X has no VM location", fix.arm64Target)
		}
