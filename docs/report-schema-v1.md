...
| `development_strategy` | string | on classified runs | Last completed internal boundary: Android `rewrite-artifact-ready`/`rewrite-plan-ready`, or iOS `macho-relocation-only` for the structural lane. |
...
| `runtime_strategy` | string | after runtime validation | Accurate runtime build/validation strategy, currently `ndk-r29-et-rel-validated` for Android or `ios-arm64-relocated-entry` for the limited iOS relocation lane. The iOS value does not mean a VM interpreter was injected. |
