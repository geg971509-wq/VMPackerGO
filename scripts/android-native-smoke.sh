...
INJECTOR="${ANDROID_NATIVE_INJECTOR:-auto}"
...
"$PACKER" -target android -func protected_calc -debug \
  -android-mode native -injector "$INJECTOR" -profile compat \
  -report "$OUTPUT.report.json" \
  -o "$OUTPUT" \
  "$INPUT" > "$BUILD_DIR/native_bin/$INPUT_NAME.pack.log"
