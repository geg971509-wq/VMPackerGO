...
Goal: make the already-generated runtime `.eh_frame` entries discoverable from the final rewritten ELF by extending the target's existing GNU unwind index, without yet enabling local C++ landing bridges.
...
The final writer copies runtime `.eh_frame` bytes into an appended read-only `PT_LOAD`, but does not update `.eh_frame_hdr` or `PT_GNU_EH_FRAME`. Therefore an existing target GNU unwind index does not describe the newly appended runtime FDEs.

This phase fixes that index/discovery layer for targets which already publish a GNU EH frame header. It does not claim Android unwinder behavior is complete until the required physical-device gate passes.

### Corrected scope constraints

- If the target has exactly one `PT_GNU_EH_FRAME`, parse it and integrate runtime FDEs.
- If the target has no `PT_GNU_EH_FRAME`, leave the current host rewrite behavior unchanged in this phase and keep the release/unwind gate open. Do **not** silently synthesize a new PHDR here.
- Reject duplicate `PT_GNU_EH_FRAME` entries.
...
- Reject duplicate initial-location keys, unsupported/truncated original header encodings, out-of-range signed 32-bit deltas, or a selected integration path where the final runtime `.eh_frame` cannot be parsed into FDEs.
...
Add a `gnuEHFramePlan` to `RewritePlan` with original PHDR index, original header model, reserved R-segment offset, final VA/file offset, and header size.

Planning flow for a target with a GNU EH header:
1. reserve runtime allocatable sections as today;
2. parse the original `PT_GNU_EH_FRAME` bytes and count runtime FDEs from the runtime image;
3. reserve exact final header size using `original entry count + runtime FDE count`;
4. place segments;
5. apply runtime relocations;
6. parse the now-relocated runtime `.eh_frame` from the planned R segment;
7. combine original and runtime FDE index entries and materialize the canonical final header into the reserved R-segment range;
8. generate the final program-header table with the existing `PT_GNU_EH_FRAME` entry redirected to the new header.

The final header's primary `.eh_frame` pointer remains the original target `.eh_frame` address from the original header. Runtime FDEs may live in the appended R segment because each search-table entry carries an explicit resolved FDE address.
...
- duplicate `PT_GNU_EH_FRAME` rejects;
- no-header fixture retains current behavior and does not claim unwind integration;
...
- exercise the GNU-EH merge helper against the real relocated runtime `.eh_frame` shape;
- verify the final header model includes runtime `vm_entry_token` / `vm_native_call` FDE initial locations.

## 5. Exit criteria

- for supported targets with an existing GNU EH header, appended runtime FDEs are represented in the final GNU EH search table;
- existing target FDE index entries are retained;
- targets with no GNU EH header are not falsely marked closed;
...
Phase 15 can add missing-header PHDR synthesis if needed by corpus evidence, then build per-call invoke wrappers around the existing `vm_native_call`, add original landing identity to `InvokeThunk`, generate custom personality/LSDA FDEs, and route landing state back into the VM. The physical Android unwinder test remains the hard release gate.
