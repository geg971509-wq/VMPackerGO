# VMP Phase 9 Rewrite Writer Implementation Plan

Status: consensus successor plan for current HEAD `bf194bf` (`fix/call-vm-nested`).

Scope: close the current `rewrite-plan-ready` boundary by materializing the already validated `RewritePlan` into a complete in-memory ELF artifact. `archive/**` stays read-only. Decoder/runtime capability expansion is explicitly outside this writer stage.

Phase-label note: canonical product numbering in `PRODUCTIZATION_HANDOFF.md:206-228` keeps the complete plan-first ELF writer under **Phase 8** and reserves **Phase 9** for corpus/device differential work. The current code sentinel `internal/elf/options.go:73` and prior planning commit `5395be8` used “Phase 9 rewrite writer” as an implementation-workflow label for the final applicator. This successor plan keeps that label only for continuity; implementation-facing docs must not renumber the product phases. The concrete deliverable is the final in-memory applicator that closes the Phase-8 writer umbrella.
...
- current writer sentinel and result scaffolding: `internal/elf/options.go:59-73,103-148`;
- rewrite-plan model: `internal/elf/rewrite_plan.go:28-110`;
- complete plan construction and validation handoff: `internal/elf/rewrite_plan.go:125-189`;
...
- failure report preserves populated result metadata: `internal/report/report.go:132-152`;
- existing publication transaction: `internal/publish/publish.go:52-90`;
- historical phase naming: `PRODUCTIZATION_HANDOFF.md:206-228`, `docs/android-arm64-test-plan.md:19-40`.
...
1. Reject nil/internally impossible writer input and validate direct-application preconditions only. At minimum: ELF64 header is present; current `e_phoff/e_phnum` match `phoffBefore/phnumBefore`; `len(tableData) == int(phnumAfter)*elf64ProgramSize`; every segment/PHDR range is overflow-safe and representable by `int`; every entry-patch range is overflow-safe and remains fully inside the original input; and, when `programHeaders.relocated` is true, exactly one planned segment contains the relocated PHDR range and its embedded bytes agree with `programHeaders.tableData`. These are consistency/bounds checks, not replanning.
...
Writer-level checks are allowed only to prevent unsafe copies or prove that fields needed for direct application are self-consistent. They must not select alternate offsets, rebuild PHDRs, regenerate entry trampolines, or otherwise become a second planner. All validation that can fail before allocation/copy should run before the scratch artifact is created; any later failure must return `nil, error`, never the partially written scratch slice.
...
2. Populate the stable pre-artifact result metadata already known at this point: `Functions`, `OpcodeMapDigest`, `RuntimeStrategy`, and `DevelopmentStrategy = "rewrite-plan-ready"`. Do **not** retain the hidden `rewritePlan` pointer in the result.
3. Call `applyRewritePlan(req.Input, plan)`.
4. Structurally validate the produced artifact with `parseELFMetadata(artifact, normalizedMode)` and confirm the reparsed target kind still matches the analyzed target; close the returned metadata/file exactly as existing parser callers do.
5. Only after writer + reparse validation succeeds:
   - change `DevelopmentStrategy` to `"rewrite-artifact-ready"`;
   - assign `Result.Artifact = artifact`;
   - return `nil` error.
6. On writer or final-reparse error, return the populated metadata with `DevelopmentStrategy = "rewrite-plan-ready"` and an empty `Artifact`; this preserves accurate failure-report observability through `internal/report.Report.Fail` (`internal/report/report.go:132-152`).
7. Remove the temporary boundary-only `ErrRewriteWriterRequired` and `Result.rewritePlan` state once all callers/tests are migrated; both are internal scaffolding, not post-writer product contracts.
...
- change the Tier 2 prerequisite to factual capability wording: the plan-first writer is fully implemented and Tier 1 host validation passes; keep canonical product Phase 8/9 numbering unchanged;
...
Record the dirty-file set. At planning time the only unrelated untracked paths are `.claude/` and `.jspace/`; do not reset, revert, stage, or modify them. `archive/**` remains a non-touchpoint for this writer stage.
...
- invalid writer/reparse path after planning: error, empty `Artifact`, `DevelopmentStrategy == "rewrite-plan-ready"`;
...
- Tier 2 prerequisite says the plan-first writer is implemented and Tier 1 host validation passes, without renumbering product phases;
...
15. **Accurate reporting:** success reports use `development_strategy: rewrite-artifact-ready`, include output hash/status success, and do not mention the writer-required sentinel; writer/reparse failures after planning retain `development_strategy: rewrite-plan-ready` with no output hash.
...
### Prior approved consensus inherited from `5395be8`

The earlier Phase 9 plan at commit `5395be8` already completed Planner -> Architect -> Critic and was approved. Current HEAD `bf194bf` implemented the validated `RewritePlan` boundary but did not implement the writer, so the architectural decision remains valid. This successor plan carries that approved decision forward instead of replacing it.

### Current delta review after `bf194bf`

The current-code audit requires four tightenings before execution:

1. Restore the prior plan's exact eight-step writer mutation order; the new draft had compressed it too far.
2. Preserve `rewrite-plan-ready` on writer/reparse failure so failure reports identify the last completed boundary, while success alone advances to `rewrite-artifact-ready`.
3. Treat relocated-PHDR `tableData` versus embedded read-only-segment bytes as a direct consistency invariant before materialization.
4. Clarify Phase 8/Phase 9 nomenclature explicitly rather than rewriting historical handoff numbering.

### Current Critic verdict

Critic verdict: **APPROVE**. No blocking issues. Two nonblocking improvements were adopted before commit:

- name `parseELFMetadata(artifact, normalizedMode)` as the exact final structural reparse helper;
- name the new `internal/elf/rewrite_writer_test.go` explicitly in implementation touchpoints.

Representative execution simulation passed: an executor can start from the plan without inventing mutation order, PHDR handling, failure-state semantics, parser choice, or publication ownership.
...
## 9) Available-agent-types roster and reasoning-by-lane
...
- writer/PHDR implementation: `executor`, medium-to-high reasoning; one owner only;
- regression matrix: `test-engineer`, medium reasoning with exact byte assertions;
- unexpected writer/reparse failure: `debugger`, high reasoning;
- architecture contradiction only: `architect`, high reasoning;
- final acceptance and SHA-bound evidence: `verifier`, high reasoning;
- staging/commit isolation: `git-master`, high reasoning.
...
Launch hints after this planning commit:
- Ralph: `$ralph "Execute .omx/plans/vmp-phase9-rewrite-writer-implementation-plan.md and .omx/plans/test-spec-vmp-phase9-rewrite-writer.md"`
- Team, only if parallelism is justified: `omx team 2:executor "Implement the Phase 9 writer from .omx/plans/vmp-phase9-rewrite-writer-implementation-plan.md with disjoint writer/test ownership"`, followed by a separate verifier pass.
...
Only these four ignored `.omx/` files may be force-staged:

- `.omx/context/vmp-phase9-rewrite-writer-implementation-20260831T225637Z.md`
- `.omx/plans/vmp-phase9-rewrite-writer-implementation-plan.md`
- `.omx/plans/prd-vmp-phase9-rewrite-writer.md`
- `.omx/plans/test-spec-vmp-phase9-rewrite-writer.md`
...
Stop if anything else is staged. `.claude/` and `.jspace/` remain untracked and must not enter the commit.

Lore commit intent:

```text
Freeze the Phase 9 writer implementation contract before execution.

Constraint: Preserve the validated RewritePlan boundary and keep local workflow artifacts and Phase 10 capability work outside this planning commit.
Rejected: Recompute ELF layout inside the writer | it would reopen settled Phase 8 decisions and weaken failure atomicity.
Confidence: high
Scope-risk: narrow
Directive: Phase 9 applies an immutable RewritePlan only; publication remains owned by internal/publish.
Tested: Consensus review, current file-reference validation, targeted package baseline, and staged-diff checks.
